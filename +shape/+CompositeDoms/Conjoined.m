classdef Conjoined< shape.C2boundary
    ...
    methods
      function obj = Conjoined(scales,nbPoints,r1,r2,dspl)
            a = 0.5 ; 
            if nargin < 3
               r1 = 0.7;
               r2 = 0.5; 
            ...
            selector2 = 0:1/(floor(nbPoints*fr2)):1-1/(floor(nbPoints*fr1)) ;
            
            points = scales*[[r1*cos((2*pi+2*phase2)*selector1+phase1); r1*sin((2*pi+2*phase2)*selector1+phase1) ],...
                     [r2*cos((2*phase4)*fliplr(selector2)+phase3)+a; r2*sin((2*phase4)*fliplr(selector2)+phase3)]];     
            points = circshift(points,[0,3*(floor(nbPoints/(14/5)))]); %this is so that the begining and end are on an edge
            points = points - points(:,1) ;
            
            theta = 0:(1/length(points)):(1-(1/length(points))) ;
            [points, tvec, avec, normal] = shape.C2boundary.rescale(points, theta, nbPoints, [], dspl); 

            obj = obj@shape.C2boundary(points, tvec, avec, normal, [0;0], 'Conjoined');
